...
const submitProject = async () => {
  if (projectUploadType.value === 'link') {
    ...
    try {
      const fileCol = currentStep.value === 9 ? 'Final_Project_Code' : (currentStep.value === 7 ? 'Project3_Code' : (currentStep.value === 5 ? 'Project2_Code' : 'Project1_Code'));
    ...
    reader.onload = async (e) => {
      ...
      const projName = currentStep.value === 9 ? 'final project' : (currentStep.value === 7 ? 'mini project C' : (currentStep.value === 5 ? 'mini project B' : 'mini project A'));
      const fileNameStr = `${studentData.value.name}_${studentData.value.school}_grupA_${projName}_${projectFile.value.name}`;
      const fileCol = currentStep.value === 9 ? 'Final_Project_Code' : (currentStep.value === 7 ? 'Project3_Code' : (currentStep.value === 5 ? 'Project2_Code' : 'Project1_Code'));
...
const getQuestionChoices = (question) => {
  ...
  if (Array.isArray(opts) if (Array.isArray(question.choices) && question.choices.length > 0) return question.choices;if (Array.isArray(question.choices) && question.choices.length > 0) return question.choices; opts.length > 0) return opts;
...
const isStepFinished = (stepId) => {
  return true; // DIBUKA SEMENTARA UNTUK QA
  /*
  ...
  if (stepId === 2 || stepId === 5 || stepId === 7 || stepId === 9) {
    // TODO: Buka komentar ini saat akan push ke Git agar kewajiban upload aktif
    const projCol = stepId === 9 ? 'Final_Project_Code' : (stepId === 7 ? 'Project3_Code' : (stepId === 5 ? 'Project2_Code' : 'Project1_Code'));
    ...
    <div v-if="!isLoggedIn" class="login-overlay">
      <div class="login-card">
        ...
        <div class="login-form-panel">
          <span class="login-step">Materi Grup A - SMP</span>
        ...
        <section class="step-panel" v-for="(data, step) in courseData" :key="step" :id="'step-' + step" v-show="currentStep === parseInt(step)">
          ...
          <div class="reading-container" v-if="data.summaryHtml">
            <details class="lesson-reading-accordion" :open="isDesktop ? true : undefined">
              ...
              <div class="project-upload-area" v-if="currentStep === 2 || currentStep === 5 || currentStep === 7 || currentStep === 9" style="background: var(--paper); border-radius: 16px; padding: 32px; margin-top: 32px; border: 2px dashed var(--line); text-align: center; box-shadow: var(--shadow);">
